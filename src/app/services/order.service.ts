...
export class OrderService {
  ...
  getOrders(id: string): Observable<any>{
    return this.http.get<any>(`http://localhost:3000/api/orders/${id}`);
  }
  ...
  getAllOrders(page: number = 1, limit: number = 10, status: string = ''): Observable<any> {
    ...
    return this.http.get<any>(`http://localhost:3000/api/orders`, { params });
  }
  
  
deleteOrder(id: string): Observable<any> {
  return this.http.delete<any>(`http://localhost:3000/api/orders/${id}`);
}
